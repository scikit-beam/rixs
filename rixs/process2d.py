...
from pyrixs import loaddata  # REPLACE
...
def make_fake_image():
    ...
    choose = ((np.exp(-(randomy-1000)**2 /5)+.002)
              > np.random.rand(len(randomy)))
...
def fit_curvature(photon_events, binx=32, biny=0.5, CONSTANT_OFFSET=500):
    ...
    """
    x_centers, offsets = get_curvature_offsets(photon_events,
                                               binx=binx, biny=biny)
...
def clean_image_std(photon_events, sigma, curvature):
    """ Remove cosmic rays and glitches based on the stardard deviation
    for each isoenergetic row. Values beyond +-sigma[i]*std are removed.
    Note that it is usefull to have sigma.size >= 3 in decreasing order.
    This is because in a single iteraction, a big glitch will affect 
    the mean and std, potentially masking a small one.
    ...
    sigma: list or array
        ...
        i.e. values beyond +-sigma[i]*std are removed. 
        The sigma[i]<=0 are ignored.
    curvature : array
        n2d order polynominal defining image curvature
        np.array([x^2 coef, x coef, offset])
    ...
    corrected_y = y - poly(x, curvature[0], curvature[1], 0.)
    y_edges = 1.0 * np.arange(np.nanmin(corrected_y)//1.0,
                              np.nanmax(corrected_y)//1.0 + 2)
    y_centers = (y_edges[:-1] + y_edges[1:])/2

    cleanI = np.copy(I)
    for sig in sigma:
        # Creating index to convert the mean and std binned arrays
        # of y_centers.size into cleanI.size
        index = np.digitize(corrected_y, y_edges)

        mean, _, _ = binned_statistic(corrected_y, cleanI,
                                      statistic='mean', bins=y_edges)
        std, _, _ = binned_statistic(corrected_y, cleanI,
                                     statistic='std', bins=y_edges)
        mean_array = mean[index-1]
        std_array = std[index-1]

        bad_indices = np.logical_or(cleanI < (mean_array - sig*std_array),
                                    cleanI > (mean_array + sig*std_array))
