...
def load_spectra(search_path, selected_file_names):
    ...
    for name in selected_file_names:
        ...
        spectrum = pd.Series(data[:, 2], index=np.arange(len(data[:, 2])),
                             name=name)
...
def make_fake_spectrum(x, elastic_energy=50, noise=0):
    ...
    return y1 + y2 + noise*np.random.rand(len(x))
...
def plot_spectra(ax1, spectra, align_min=None, align_max=None):
    ...
    artists = [spec.plot(color=next(plot_color))
               for _, spec in spectra.iteritems()]
    ...
    xmin_artist, xmax_artist = [plt.axvline(x=x, color='b')
                                for x in [align_min, align_max]]
...
def plot_shifts(ax2, shifts):
    ...
    # plt.xticks(Spectra['shifts'].index, 
    # [name for name in Spectra['spectra'].columns], rotation=30)
    return shifts_line


def get_shifts(spectra, reference, align_min=-np.inf,
               align_max=np.inf, background=0.):
    ...
    """
    choose_range = np.logical_and(reference.index > align_min,
                                  reference.index < align_max)
    ref = reference[choose_range].values

    zero_shift = np.argmax(np.correlate(ref-background,
                                        ref-background, mode='Same'))

    shifts = []
    names = []
    for name, spectrum in spectra.iteritems():

        choose_range = np.logical_and(spectrum.index > align_min,
                                      spectrum.index < align_max)
        spec = spectrum[choose_range].values

        cross_corr = np.correlate(ref-background, spec-background,
                                  mode='Same')
...
def get_shifts_w_mean(spectra, reference, 
                      align_min=-np.inf, align_max=np.inf, background=0.):
    ...
    shifts = get_shifts(spectra, reference, align_min=align_min,
                        align_max=align_max, background=background)
    reference = apply_shifts(spectra.copy(), shifts).sum(axis=1)

    return get_shifts(spectra, reference, align_min,
                      align_max, background=background)
...
def apply_shifts(spectra, shifts):
    ...
    for name, spec in spectra.iteritems():
        shifted_intensities = np.interp(spec.index - shifts[name],
                                        spec.index, spec.values)
...
def run_test(search_path='test_data/*.txt'):
    ...
    align_min = 10
    align_max = 70
    artists = plot_spectra(ax1, spectra, align_min=align_min,
                           align_max=align_max)
    first_spectrum_name = spectra.columns[0]
    shifts = get_shifts(spectra, spectra[first_spectrum_name],
                        align_min=align_min, align_max=align_max,
                        background=0.5)
    # shifts = get_shifts(spectra, spectra[first_spectrum_name],
    # align_min=align_min, align_max=align_max, background=0.5)
